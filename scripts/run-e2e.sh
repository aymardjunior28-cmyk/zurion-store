...
echo "== IMG FILES ==" >> "$OUT"
ls -1 assets/images/demos/demo-3/products/ 2>&1 | grep -E 'product-[0-9]+\.jpg' | head -8 >> "$OUT"
echo "COUNT: $(ls -1 assets/images/demos/demo-3/products/ 2>/dev/null | grep -cE 'product-[0-9]+\.jpg')" >> "$OUT"

echo "== CLEAN DB + SEED ==" >> "$OUT"
rm -f data/zurion.sqlite data/zurion.sqlite-* 2>/dev/null
timeout 90 node scripts/seed.js >> "$OUT" 2>&1
echo "SEED_EXIT=$?" >> "$OUT"

node -e '
const D=require("sqlite3");const db=new D.Database("data/zurion.sqlite");
...
sleep 1
setsid nohup node server.js > /tmp/server.log 2>&1 &
echo "SERVER_PID=$!" >> "$OUT"
sleep 4
echo "== SERVER LOG ==" >> "$OUT"
