'use strict';

const { User } = require('../models');
const { hashPassword, comparePassword } = require('../utils/password');
const { signToken, cookieOptions, COOKIE_NAME } = require('../middlewares/auth');
const cartService = require('../services/cart.service');

/** POST /api/auth/register */
/* ═══════════════════════════════════════════════════════════════════════════
 *  CONTRÔLEUR AUTH — inscription, connexion, déconnexion, profil
 * ═══════════════════════════════════════════════════════════════════════════ */

/** POST /api/auth/register — crée un client, pose le cookie de session JWT. */
async function register(req, res, next) {
  try {
    const { firstName, lastName, email, phone, password } = req.body;
    const existing = await User.findOne({ where: { email } });
    if (existing) return res.status(409).json({ error: 'Un compte existe déjà avec cet e-mail.' });

    const user = await User.create({
      firstName,
      lastName,
      email,
      phone: phone || null,
      passwordHash: await hashPassword(password),
      role: 'customer',
    });

    const token = signToken(user);
    res.cookie(COOKIE_NAME, token, cookieOptions());

    // Fusion du panier invité au compte
    await cartService.mergeGuestIntoUser(req.headers['x-cart-token'], user.id);

    return res.status(201).json({
      user: { id: user.id, firstName, lastName, email, role: user.role },
    });
  } catch (err) {
    return next(err);
  }
}

/** POST /api/auth/login */
/** POST /api/auth/login — vérifie les identifiants (message générique), pose le cookie JWT. */
async function login(req, res, next) {
  try {
    const { email, password } = req.body;
    const user = await User.findOne({ where: { email } });
    // Message générique pour ne pas révéler si l'adresse existe
    const ok = user && (await comparePassword(password, user.passwordHash));
    if (!ok) {
      return res.status(401).json({ error: 'Identifiants incorrects.' });
    }
    const token = signToken(user);
    res.cookie(COOKIE_NAME, token, cookieOptions());

    await cartService.mergeGuestIntoUser(req.headers['x-cart-token'], user.id);

    return res.json({
      user: { id: user.id, firstName: user.firstName, lastName: user.lastName, email: user.email, role: user.role },
    });
  } catch (err) {
    return next(err);
  }
}

/** POST /api/auth/logout */
/** POST /api/auth/logout — supprime le cookie de session. */
function logout(req, res) {
  res.clearCookie(COOKIE_NAME, { path: '/' });
  return res.json({ ok: true });
}

/** GET /api/auth/me */
/** GET /api/auth/me — retourne le profil de l'utilisateur authentifié. */
async function me(req, res, next) {
  const u = req.user;
  return res.json({
    user: {
      id: u.id,
      firstName: u.firstName,
      lastName: u.lastName,
      email: u.email,
      phone: u.phone,
      role: u.role,
    },
  });
}

module.exports = { register, login, logout, me };