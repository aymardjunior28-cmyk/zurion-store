 ...
 *  Cycle de vie : créée → paiement_confirmé → préparation → expédition → livraison → terminée
 *  + statut annulé (annulée).
 *  - addressSnapshot : instantané JSON de l'adresse au moment de la commande
 *    (l'historique reste fidèle même si le client modifie son adresse ensuite).
 *  - couponCode / discount : code promo appliqué + remise en FCFA.
 * ═══════════════════════════════════════════════════════════════════════════ */
const { DataTypes } = require('sequelize');
const { sequelize } = require('../config/db');

/** Commande avec cycle de vie : créée → paiement_confirmé → préparation → expédition → livraison → terminée */
